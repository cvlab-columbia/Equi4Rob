...
def get_loader(args, split, out_name=False, customized_task_set=None, nonormalize=False, use_depth=False):
            ...
            if nonormalize:
                if task_set_present:
                    ...
                    loader = torch.utils.data.DataLoader(
                        ...
                        transforms.ToTensor()
                        ]), list_dir=args.list_dir, out_name=out_name, use_depth=use_depth),
...
def get_corruption_loader(args, split, out_name=False, customized_task_set=None, nonormalize=False, corruption_name='', corruption_level=3):
    """Returns data loader depending on dataset and split"""
    dataset = args.dataset
    loader = None
    ...
    if dataset == 'cityscape':
        ...
        if nonormalize:
            if task_set_present:
                print("\nCAUTION: THE DATALOADER IS FOR MULTITASK ON CITYSCAPE\n")
                loader = torch.utils.data.DataLoader(
                    CorruptionSegList(data_dir, 'val', transforms.Compose([
                    transforms.ToTensor()
                    ]), list_dir=args.list_dir, out_name=out_name, corruption_name=corruption_name, corruption_level=corruption_level),
                    batch_size=args.test_batch_size, shuffle=False, num_workers=args.workers,
                    pin_memory=True, drop_last=True
                )
            else:
...
class SegList(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, phase, transforms, list_dir=None,
                 out_name=False, use_depth=False):
    ...
    def __getitem__(self, index):
        ...
        if self.use_depth:
            return rgb_data, depth_label, mask
        else:
            return rgb_data, segmentation_label, mask
...
class CorruptionSegList(torch.utils.data.Dataset):

    INFO = {"mean": [0.29010095242892997,0.32808144844279574,0.28696394422942517],
            "std": [0.1829540508368939, 0.18656561047509476, 0.18447508988480435]}

    def __init__(self, data_dir, phase, transforms, list_dir=None,
                 out_name=False, corruption_name='', corruption_level=3):
        self.list_dir = data_dir if list_dir is None else list_dir
        self.label_dir = data_dir
        self.data_dir = join(data_dir, 'Corruption_val', corruption_name, str(corruption_level))
    ...
    def __getitem__(self, index):
        ...
        if self.label_list is not None:
            ...
            data.append(Image.open(
                join(self.label_dir, self.label_list[index])))
        # print('data', data)
...
class CorruptionSegDepthList(torch.utils.data.Dataset):
    """
    Dataloader for getting the multitask labels from Cityscapes.
    """
    INFO = {"mean": [0.29010095242892997,0.32808144844279574,0.28696394422942517],
    ...
    def __getitem__(self, index):
        ...
        # Prepare segmentation label
        segmentation_label = Image.open(join(self.label_dir, self.label_list[index]))

        # Prepare depth label
        disparity_img_path = join(self.label_dir, im_file_path.replace("leftImg8bit","disparity"))
        depth_img = (Image.open(disparity_img_path))
        # depth_img = torch.ByteTensor(torch.ByteStorage.from_buffer(depth_img.tobytes()))
